...
interface AuthContextType {
  ...
  token: string | null;
  loading: boolean;
  login: (token: string, userData: User) => void; // <-- UPDATED: Now accepts user data
  logout: () => void;
  refreshUser: () => Promise<void>;
}
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  // --- THIS IS THE DEFINITIVE FIX FOR THE LOGIN LOGIC ---
  // The login function is now much simpler and more reliable.
  const login = (newToken: string, userData: User) => {
    // 1. Store the token for session persistence.
    localStorage.setItem('token', newToken);
    // 2. Set the Authorization header for all future requests.
    axios.defaults.headers.common['Authorization'] = `Bearer ${newToken}`;
    // 3. Set the user and token state directly from the data provided by the login API.
    setUser(userData);
    setToken(newToken);
  };
