...
const OtpVerification = () => {
  ...
  const navigate = useNavigate();
  const location = useLocation();
  const { login: authLogin } = useAuth(); // Get the updated login function
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = purpose === 'register' 
      ? `${import.meta.env.VITE_API_BASE_URL}/api/auth/verify-registration` 
      : `${import.meta.env.VITE_API_BASE_URL}/api/auth/verify-login`;
    ...
    try {
      ...
      if (purpose === 'register') {
        ...
        // The backend returns both a `token` and a `user` object.
        // We pass both directly to our updated login function.
