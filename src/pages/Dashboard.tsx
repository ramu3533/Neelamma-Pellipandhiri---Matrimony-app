...
const Dashboard = () => {
  ...
  const [selectedProfile, setSelectedProfile] = useState<MyProfile | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [myProfileRes, receivedRes, sentRes, acceptedRes, likesRes] =
          await Promise.all([
            axios.get(
              `${import.meta.env.VITE_API_BASE_URL}/api/profiles/me`,
              config
            ),
            axios.get(
            ...
            axios.get(
              `${import.meta.env.VITE_API_BASE_URL}/api/interests/sent`,
              config
            ),
            axios.get(
              `${import.meta.env.VITE_API_BASE_URL}/api/interests/accepted`,
              config
            ),
            axios.get(
              `${import.meta.env.VITE_API_BASE_URL}/api/likes/received`,
              config
            ),
          ]);
  ...
  useEffect(() => {
    if (socket) {
      const handleStatusUpdate = (updatedInterest: InterestRequest) => {
        const updateUserLists = (prev: InterestRequest[]) =>
          prev.map((i) =>
            i.interest_id === updatedInterest.interest_id
              ? { ...i, status: updatedInterest.status }
              : i
          );
        ...
        if (updatedInterest.status === "accepted") {
          setAcceptedChats((prev) => [
            ...prev.filter((c) => c.user_id !== updatedInterest.user_id),
            updatedInterest,
          ]);
        }
      };
      ...
      return () => {
        socket.off("interest_status_updated", handleStatusUpdate);
      };
    }
  }, [socket]);
  ...
  const handleCroppedImageUpload = async (croppedImageBlob: Blob) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/profiles/picture`,
        formData,
        {
          headers: {
      ...
      setMyProfile((prev) =>
        prev ? { ...prev, image: res.data.image } : null
      );
  ...
  const handleGalleryUpload = async (e: React.FormEvent) => {
    ...
    if (!galleryFiles || galleryFiles.length === 0)
      return setStatusMessage({
        type: "error",
        text: "Please select files for the gallery.",
      });
    const formData = new FormData();
    for (let i = 0; i < galleryFiles.length; i++)
      formData.append("profileImages", galleryFiles[i]);
    setStatusMessage({ type: "info", text: "Uploading to gallery..." });
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/profiles/images`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      setMyProfile((prev) =>
        prev ? { ...prev, images: res.data.images } : null
      );
      setStatusMessage({ type: "success", text: res.data.message });
  ...
  const handleDeleteImage = async (imageId: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `${import.meta.env.VITE_API_BASE_URL}/api/profiles/images/${imageId}`,
  ...
  const handleAddInterest = () => {
    if (
      newInterest &&
      myProfile &&
      !myProfile.interests.includes(newInterest)
    ) {
      setMyProfile({
        ...myProfile,
        interests: [...myProfile.interests, newInterest],
      });
      setNewInterest("");
    }
  };
  const handleRemoveInterest = (interestToRemove: string) => {
    if (myProfile)
      setMyProfile({
        ...myProfile,
        interests: myProfile.interests.filter((i) => i !== interestToRemove),
      });
  };
  ...
  const handleSaveInterests = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/profiles/interests`,
  ...
  const handleRespondInterest = async (
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/interests/respond/${
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 space-y-6">
              <div className="flex items-center space-x-4 border-b pb-6">
                ...
                <div>
                  ...
                  <label
                    ...
                    className="mt-2 text-sm font-semibold text-rose-600 hover:text-rose-700 cursor-pointer"
                  >
              ...
              <div className="space-y-4">
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          onClick={() => handleSetMainPicture(img.image_url)}
                ...
                <form onSubmit={handleGalleryUpload} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-rose-500 text-white py-2 rounded-lg hover:bg-rose-600 transition disabled:bg-gray-400"
              ...
              {activeChat ? (
                ...
                <div className="p-6">
                  ...
                  <div className="flex border-b mb-4 flex-wrap">
                    ...
                    >
                      <Heart size={18} /> Likes ({receivedLikes.length})
                    </button>
                    <button
                    ...
                    >
                      <Inbox size={18} /> Received ({receivedInterests.length})
                    </button>
                    <button
                    ...
                    >
                      <Send size={18} /> Sent ({sentInterests.length})
                    </button>
                  </div>
                    ...
                    {activeTab === "likes" &&
                      receivedLikes.map((like) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleViewProfile(like.user_id)}
                    ...
                    {activeTab === "chats" &&
                      acceptedChats.map((chat) => (
                        <button
                          ...
                          className="w-full flex items-center gap-4 p-3 bg-gray-50 rounded-lg text-left hover:bg-rose-50 transition-colors"
                        >
                          {chat.image ? (
                            <img
                              src={`${import.meta.env.VITE_API_BASE_URL}/${
                        ...
                        >
                          ...
                          {interest.status === "pending" && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  handleRespondInterest(interest, "accepted")
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleRespondInterest(interest, "rejected")
                                }
